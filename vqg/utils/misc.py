...
def save_checkpoint_mem(opt, model, infos, optimizer,optimizer_mem,memory_cell, histories=None, append=''):
    if len(append) > 0:
        append = '-' + append
    # if checkpoint_path doesn't exist
    if not os.path.isdir(opt.checkpoint_path):
    ...
    print("model saved to {}".format(checkpoint_path))
    optimizer_path = os.path.join(opt.checkpoint_path, 'optimizer%s.pth' %(append))
    memory_cell_path = os.path.join(opt.checkpoint_path, 'memory_cell%s' % (append))
    optimizer_mem_path = os.path.join(opt.checkpoint_path, 'optimizer_mem%s.pth' % (append))
    torch.save(optimizer.state_dict(), optimizer_path)
