        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if suppress_UNK and hasattr(self, 'vocab') and str(logprobs.size(1)-1) in self.vocab \
                            and self.vocab[str(logprobs.size(1)-1)] == '<unk>':
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    # suppress UNK tokens in the decoding
                    if suppress_UNK and hasattr(self, 'vocab') and str(logprobsf.size(1)-1) in self.vocab \
                            and self.vocab[str(logprobsf.size(1)-1)] == '<unk>':
